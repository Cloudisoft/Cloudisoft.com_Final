...
function PricingSection() { return ( <section className="section mt-24 text-center"> <h2 className="text-4xl font-bold">Choose Your Plan</h2> <p className="text-slate-400 mt-3">Start free. Upgrade anytime.</p> <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mt-12"> {[ { name: "Free", price: "0", features: ["2 simulations/month", "Basic reports", "Email support"], cta: "Start Free" }, { name: "Starter", price: "19.99", features: ["10 simulations/month", "Summary reports", "Basic templates", "Email support"], cta: "Start Starter" }, { name: "Pro", price: "49.99", features: ["25 simulations/month", "Dashboard", "History", "Advanced templates", "Priority support"], highlight: true, cta: "Upgrade to Pro" }, { name: "Enterprise", price: "99.99", features: ["Unlimited simulations", "Team access", "Advanced analytics", "API access"], cta: "Talk to Sales" } ].map((p, i) => ( <div key={i} className={rounded-3xl p-8 border shadow-xl ${p.highlight ? "bg-gradient-to-r from-blue-500 to-purple-500 text-white border-transparent" : "bg-cloudi-card border-slate-800"}}> <h3 className="text-2xl font-bold">{p.name}</h3> <p className="text-4xl font-extrabold mt-3">${p.price}<span className="text-lg opacity-70">/mo</span></p> <ul className="mt-4 space-y-2 text-left text-sm"> {p.features.map((f, idx) => <li key={idx} className="flex gap-2"><span>✔️</span>{f}</li>)} </ul> <button className={btn-primary w-full mt-6 ${p.highlight ? "" : ""}}>{p.cta}</button> </div> ))} </div> </section> ); } // ========================== // FAQ SECTION // ========================== function FAQSection() { const faqs = [ { q: "How accurate are the simulations?", a: "CloudiCore uses directional financial logic to estimate outcomes. It is not exact forecasting but helps you understand risk, break-even, and expected impact." }, { q: "Do simulations work for all business types?", a: "Yes — SaaS, e-commerce, agencies, local businesses, marketplaces, and more." }, { q: "Can I export the results?", a: "Yes — export any simulation as PDF, perfect for investors or internal strategy meetings." }, { q: "Do I need a credit card for the trial?", a: "No — the 7-day trial is completely free." }, { q: "Is my data private?", a: "Yes. All data is secure and tied to your Supabase-authenticated account." }, { q: "Can I upgrade later?", a: "You can upgrade to Starter, Pro, or Enterprise anytime." }, { q: "Do you offer team access?", a: "Yes — Enterprise includes multi-user teams and collaboration." } ]; return ( <section className="section mt-24"> <h2 className="text-3xl font-bold text-center mb-10">Frequently Asked Questions</h2> <div className="grid grid-cols-1 md:grid-cols-2 gap-6"> {faqs.map((f, i) => ( <details key={i} className="bg-cloudi-card/60 p-4 rounded-xl border border-slate-800"> <summary className="cursor-pointer font-semibold">{f.q}</summary> <p className="text-slate-300 mt-2">{f.a}</p> </details> ))} </div> </section> ); }
